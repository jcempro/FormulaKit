...
for (const artifact of manifest.artifacts) {
  ...
  const text = bytes.toString("utf8"); const match = /\/\*# FormulaKitSignature\/v1\n([^\n]+)\n#\*\//u.exec(text); const signature = match ? JSON.parse(match[1]) : artifact.format === "sourcemap" || artifact.file.endsWith(".json") ? JSON.parse(text).x_formulakit_signature : undefined; if (!signature) throw new Error(`DIST_SIGNATURE_MISSING:${artifact.file}`); if (JSON.stringify(signature) !== JSON.stringify(artifact.signature)) throw new Error(`DIST_SIGNATURE_DIVERGENT:${artifact.file}`);
